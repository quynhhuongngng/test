			...
			<span style="font-weight: 700;"> 
				...
				<c:choose>
					<c:when test="${cart.amount > 0}">
 						Tổng tiền: <span style="color: #CC3300"><f:formatNumber value="${cart.amount}" pattern="#,###" /> VNĐ</span>
					</c:when>
					<c:otherwise>
					...
					<c:when test="${cart.amount > 0}">
						<c:forEach var="p"
							...
							<tr data-id="${p.id}" data-price="${p.unitPrice}"
								...
								<td style="padding: 20px 60px 20px 30px; width:700px;'">
									<p style="font-size: 15px; font-weight: bold;">${p.name}</p>
									...
									<c:choose>
										...
										<c:otherwise>
											<p style="font-weight: bold; ">
												<span style="text-decoration: line-through; color: Gray;"><f:formatNumber value="${p.unitPrice}" pattern="#,###" />VNĐ</span> 
												<span
													style="vertical-align: super; background-color: red; border-radius: 50%; padding: 8px; color:white;" >
								...
								<td style="padding: 50px 0px 40px 0px;">
									<input id="num" value="${p.quantity}" type="number" min="1"
									style="width: 60px; height: 30px; font-size: 16px; border: 2px solid black;">
								</td>
