						...
						<div class="row">
							<div class="col-sm-6" data-id="${prod.id}">
								<button class="theo-vao-gio btn-add-to-carts"
									data-toggle="modal" data-target="#myModals">THÊM VÀO GIỎ
								</button>
							</div>
...
<div id="comments-container">
  <section id="comment--new" class="comment">
	...
	<form:form action="addCommentProduct/${prod.id}" modelAttribute="entity">   								
    	<form:textarea path="contentOfComment" style="width: 100%; height: 100px;" required="required" placeholder="Đăng nhập để bình luận"/>         
	    <br><br>  
  ...
  <ul id="comments">
   <c:forEach var="comment" items="${comments}" begin="0" end="4">
    <li class="comment" >
      <div class="avatar">
        <a href="#">
          <img src="/static/images/user/${comment.user.photo}" style="width: 55px; height: 55px; border-radius:50%"
               alt="${comment.user.fullname} photo avatar" class="avatar__img">
        </a>
      </div>
      ...
      <div class="comment__content">
        <header>
          <a href="#">
            <span class="user__link">${comment.user.fullname }</span>
          </a>
          <span class="comment__pub-date"> ${comment.commentedDate}</span>
